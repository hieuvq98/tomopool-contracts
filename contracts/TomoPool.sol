...
contract CandidateContract {
    ...
    address payable constant public validator = payable(0x0000000000000000000000000000000000000088);
    ...
    bool public isRequireReferralLock = true;
    uint256 public requireLockAmount = 25000 ether;
    ...
    constructor (string memory _candidateName, address _coinbase) {
        ...
        teamAddr = payable(msg.sender);
    }

    modifier ownerLockAmount() {
        if (isRequireReferralLock) {
            require(getCurrentStakerCap(referralAddress) >= requireLockAmount);
        }
        _;
    }
    function setRequireReferralLock(bool _lock) public onlyTeam {
        isRequireReferralLock = _lock;
    }
    function setRequireLockAmount(uint256 _amount) public onlyTeam {
        requireLockAmount = _amount;
    }
    ...
    function stake() public payable validMaxCap(msg.value) ownerLockAmount {
    ...
    function transferStake(address to, uint256 _amount) external onlyStaker {
        if (msg.sender == referralAddress && isRequireReferralLock) {
            require(getCurrentStakerCap(msg.sender).sub(_amount) >= requireLockAmount);
        }
    ...
    function resignByTeam() public onlyTeam {
        resignInternal();
    }
    ...
    function setLastWithdrawEpochOfStaker(address _staker, uint256 _epochNumber) external onlyTeam{
        StakerRewardWithdrawState[_staker] = _epochNumber;
    }
    ...
    function buyStake(address payable _sellStaker) external payable {
        ...
        if (remaining >= requiredNormalTomo) {
            ...
            payable(msg.sender).transfer(remaining.sub(requiredNormalTomo));
        ...
        payable(msg.sender).transfer(feePerEach);
        //transfer stake
